//! Identity types for the island model.

use serde::{Deserialize, Serialize};

/// A dynamic simulation member the Cartographer partitions — a Player today, an NPC
...
/// An island: the single runtime authority over a connected group of actors and
/// the chunks their activity spans. Ids are minted monotonically by the Cartographer
/// and survive merges (the survivor keeps the lower id) so worker assignment and
/// observers can track an island across topology changes where possible.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]
pub struct IslandId(pub u64);

/// A worker thread that ticks a set of assigned islands.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, PartialOrd, Ord)]

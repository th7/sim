defmodule GameCore.ChunkPersistenceTest do
  ...
  test "join puts a brand-new Player at the origin" do
    chunk =
      start_supervised!(
        {Chunk, coord: {0, 0}, auto_tick: false, auto_flush: false, repo: StubRepo}
      )
    ...
    assert %{players: %{"newbie" => %{x: +0.0, y: +0.0}}} = Chunk.snapshot(chunk)
  end
  ...
  test "leave flushes the Player's last position to the repo" do
    ...
    assert [{{0, 0}, [%{username: "alice", x: x, y: +0.0}]}] = StubRepo.flushes()
    assert x > 0.0
  end

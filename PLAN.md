...
- **Portal as a new entity type.** Not a Structure (Structures are player-placed; Portals are not). New `Portal` component with a `direction :: :into_instance | :out_of_instance` field. `GameCore.Worldgen.portals/1` is a pure deterministic function returning the positions of all Portals in a chunk — v1 places exactly one `{:into_instance, :dungeon}` Portal at `(4000, 4000)` (quarter-offset from chunk centre) of Overworld chunk `{0, 0}` and `[]` everywhere else, so the spawn-on-first-connect position at chunk centre doesn't immediately overlap the Portal. Chunks hydrate Portals on activation alongside Resource nodes. Snapshot extension: new `portals: %{...}` key parallel to `resource_nodes` / `structures`; wire id format `"portal:<type>:<x>:<y>"`. No DB rows — Portals are stateless.
...
- **Entry sequence.**
  1. Overworld chunk `A` detects overlap, calls `Session.enter_instance(session, A_coord, A_portal_pos)`.
  2. Session calls `Instances.start_new()` → fresh per-Instance supervisor + 9 Instance chunks with Null repo.
  3. Session calls `Chunk.take_components_for(A_pid, username, spawn_pos, save_pos)` — the source flushes `save_pos` to DB (post-exit offset, not the literal pre-entry position; see Disconnect bullet) and returns the entity's components with Position overridden to `spawn_pos`. Session then `Chunk.migrate_in(dest_pid, ...)` into the Instance center chunk. `ChunkMigration.cross/6` is *not* used here — that handles same-realm Boundary crossings; cross-realm needs the Position override and save-pos flush.
  ...
  6. Session broadcasts `{:relocated, %{realm: %{kind: "instance", id: id}, coord: [1, 1]}}` on PubSub topic `player_events:<username>`; `PlayerChannel` (subscribed) pushes a `relocated` event to the client. The client computes the new 9-topic list from realm + coord.
  7. Client unsubs the 9 Overworld snapshot topics, subs the 9 Instance topics `instance:<id>:chunk:0:0` … `instance:<id>:chunk:2:2`.

- **Exit sequence.** Symmetric. Instance chunk's overlap check fires `Session.exit_instance(session)`; Session uses the same `take_components_for` + `migrate_in` pair to move the entity back to the cached `return_to` chunk with Position overridden to `A_portal_pos − (1000, 0)`; WarmSet rebuilt around the Overworld entry chunk; `Instances.terminate(id)` stops the per-Instance supervisor; client cycles snapshot subscriptions back to the 9 Overworld topics.

- **Disconnect mid-Instance.** `Session.terminate/2` extended: if `realm` matches `{:instance, id}`, call `Instances.terminate(id)` **before** `WarmSet.release_all` — eager teardown, no ghost-Instance left to idle out. On entry, the source Overworld Chunk flushes the Player's position as `save_pos = portal_pos − (1000, 0)` (one cell west of the Portal cell) rather than their literal pre-entry position; that way reconnect-after-mid-Instance-disconnect lands the Player *next to* the Portal, not *on* it (which would otherwise loop them back into a fresh Instance on the first tick after Chunk.join). **Documented limit**: any in-Instance state change is lost on disconnect — irrelevant for Phase 9 (nothing in the Instance modifies inventory), but future combat/loot phases will need an explicit story.
...
- **Tests.** ExUnit for `Worldgen.portals/1` and `InstanceWorldgen.portals/1` (pure-function); `Chunks.whereis/2` realm dispatch; `Session.enter_instance/2` and `Session.exit_instance/1` (state transitions, position override, WarmSet rebuild, supervisor lifecycle); `MovementSystem` bounds clamp; `Instances.start_new/0` + `Instances.terminate/1` lifecycle; eager-teardown on `Session.terminate/2` when realm is Instance; `Session.build/3` returns `:no_build_in_instance` in Instance. Playwright `phase9.spec.ts`: walk to Portal at `(4000, 4000)`, assert `realm()` flips to `instance`, assert player position lands at Instance spawn cell, walk to return-Portal at Instance-local `(24000, 24000)`, assert `realm()` flips back to `overworld`, assert player position lands ~one cell west of entry Portal. Disconnect-mid-Instance regression: open in-Instance, drop the page, reconnect, assert `realm()` is `overworld` and player position is near the saved post-exit-offset cell. Multi-Player isolation: two clients walk into the same Portal cell simultaneously, assert distinct Instance ids and that neither client sees the other inside.

**Done when**: walk to the Portal near the centre of Overworld chunk `{0,0}` (placed at the chunk's quarter-offset so the spawn-on-first-connect position doesn't sit on it), get smoothly migrated into a fresh 3×3 Instance with a distinct visual, walk around (bounded by clamp at the perimeter), step onto the return-Portal, get migrated back to the Overworld at the entry Portal cell. Disconnect mid-Instance, reconnect — you're back near the Portal, the Instance is gone (no leaked processes, no leaked Registry keys).

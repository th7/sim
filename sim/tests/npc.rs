...
fn player_kills_deer_into_carcass_then_harvests_meat_and_hide() {
    ...
    sim.enqueue_action("alice", Action::Damage { target: deer.clone() }, 0, 0);
    sim.enqueue_action("alice", Action::Damage { target: deer }, 0, 0);
    sim.tick();
    ...
    sim.enqueue_action("alice", Action::Harvest { target: carcass }, 0, 0);
    sim.tick();
...
fn attacked_wolf_flees_when_not_hungry() {
    ...
    sim.enqueue_action("alice", Action::Damage { target: npc_wid(&sim, NpcKind::Wolf) }, 0, 0); // provoke it
    sim.tick();

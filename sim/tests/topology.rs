...
//! hysteresis — driven through the Cartographer and checked against the canonical
//! partition oracle.

use sim::geometry::ChunkCoord;
use sim::ids::ActorId;
use sim::cartographer::{canonical_partition, Cartographer, TopologyEvent};
...
/// Assert the Cartographer's partition equals the oracle's, grouping actors the same
/// way (ignoring island ids).
fn assert_matches_oracle(lab: &Cartographer, homes: &BTreeMap<ActorId, ChunkCoord>) {
    let oracle = canonical_partition(homes);

    // Group actors by their Cartographer island id.
    let mut by_island: BTreeMap<_, BTreeSet<ActorId>> = BTreeMap::new();
    for (&actor, _) in homes {
        let iid = lab.island_of(actor).expect("actor is clustered");
...
fn merge_survivor_keeps_lower_id() {
    ...
    let cid_a = lab.island_of(a).unwrap();
    let _ = lab.insert_actor(b, c(4, 0));
    let cid_b = lab.island_of(b).unwrap();
    assert!(cid_a < cid_b);

    lab.move_actor(a, c(2, 0));
    // Both actors now live under the lower of the two original ids.
    assert_eq!(lab.island_of(a), Some(cid_a));
    assert_eq!(lab.island_of(b), Some(cid_a));
}
...
fn three_way_merge_collapses_to_one() {
    ...
    // Move the middle actor so all three chains overlap → single island.

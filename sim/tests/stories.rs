...
// The verb *logic* is reached through `Sim`'s synchronous verb methods — the
// right layer for the error-reason assertions, and a synchronous match with no
...
mod instances {
    ...
    fn an_instance_has_no_resource_nodes_or_structures() {
        ...
        let no_gatherables = sim.entity_states(realm)
            .values()
...
mod damage_structure {
    ...
    fn a_destroyed_structure_no_longer_blocks_movement() {
        ...
        assert!(
            !sim.entity_states(Realm::Overworld).contains_key(&WireId("structure:3500:3000".into())),
